...
- RAM discovery now runs one search per speed tier (`RAM_SPEEDS`, default `5000,5200,5600,6000,6400`) against eBay and every scraped retailer, instead of a single fixed-speed query, so cheaper/faster tiers aren't crowded out by whichever speed a generic search happens to rank first. Customize the wording with `RAM_QUERY_TEMPLATE` (default `"32GB DDR5 desktop memory {speed}MHz"`).
