...
- Each broad retailer search runs independently on its own schedule (`STORES` in `dealbot/config.py`) — currently 5 minutes for stores that haven't shown blocking (Newegg, Best Buy page, B&H, Central Computers) and 15 minutes for stores that have actually hit HTTP 403 (Micro Center, Adorama, Antonline). A 403/429 causes only that store to additionally wait a random 30–60 minutes on top of its own schedule.
